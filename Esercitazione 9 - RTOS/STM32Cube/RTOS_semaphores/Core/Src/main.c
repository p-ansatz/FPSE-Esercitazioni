...
void producer_thread(void *argument){
	...
	for(;;){
		...
		// Put Temperature on message queue
		osMessageQueuePut(msg_queueHandle, &temp, 0U, 0U);
...
*/
void consumer_thread(void *argument){
	char uart_data_buff[10]; //Fixed Size!
	osStatus_t status;
	float msg;
	/* Infinite loop */
	for(;;){
		...
		status = osMessageQueueGet(msg_queueHandle, &msg, NULL, 0U);
		if (status == osOK) {
			ftoa(msg, uart_data_buff, 4); // Float to Array!
		}
