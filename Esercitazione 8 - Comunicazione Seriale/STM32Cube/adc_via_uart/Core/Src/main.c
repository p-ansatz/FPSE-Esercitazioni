...
float temp_mv;
float temp;
/* UART array of byte to send (used as a string)*/
...
int main(void){
	...
	/* Starting ADC/TIMER */
	HAL_TIM_Base_Start(&htim2); // Start Timer - no interrupt mode

	NVIC_SetPriority(ADC_IRQn, 0); //Interrupt configuration for ADC
